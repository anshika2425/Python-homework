...
# building.run_elevators(2,2)
    ...
    def fire_alarm(self):
        print("\nFIRE ALARM! Returning all elevators to the bottom floor:")
        for idx, e in enumerate(self.elevators):
            if e.current_floor != self.bottom_floor:
                print(f"\nMoving elevator {idx+1} to bottom floor:")
                e.go_to_floor(self.bottom_floor, idx +1)
